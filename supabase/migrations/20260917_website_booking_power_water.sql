-- Add has_power / has_water to the website booking RPC. Dropped and recreated
-- rather than overloaded so PostgREST has exactly one function to resolve.
-- Both new params default to false, so an older client keeps working.
drop function if exists public.create_website_booking(text, text, text, text, text, date, time, text);
...
begin
  ...
  insert into public.bookings (
    ...
    address, booking_date, booking_time, has_power, has_water, price, notes,
...
revoke all on function public.create_website_booking(text, text, text, text, text, date, time, text, boolean, boolean) from public;
grant execute on function public.create_website_booking(text, text, text, text, text, date, time, text, boolean, boolean) to anon, authenticated;
