...
import { Booking, BookingStats, ServiceLocation } from '@/types/booking';
...
interface ChannelBookingsPageProps {
...
/**
 * Shared implementation behind the Mobile and Shop pages.
 *
 * Deliberately a mirror of the Operations Dashboard: same stat cards, same
 * date/day filter controls, same read-only booking list. The only differences
 * are which channel it reads and that its stats are scoped to that channel.
 *
 * Bookings are created on the Admin page, which carries the Service Location
 * selector, so these pages are view-only exactly like the Dashboard.
 */
export default function ChannelBookingsPage({
  location,
  heading,
  description,
  badgeLabel,
  ...
  const loadData = useCallback(async () => {
    try {
      setError(null);
      // Every booking, not just scheduled ones: the stat cards need completed
      // rows too, exactly as the Dashboard's booking_stats view counts them.
      const data = await getBookings();
      setBookings(data);
    } catch (err: any) {
      console.error('[ChannelBookingsPage loadData error]:', err);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToBookings(() => {
      console.log('[ChannelBookingsPage Realtime] Booking change detected. Refreshing data...');
      loadData();
    });
  ...
  // Rows created before the shop channel existed carry the 'mobile' default, so
  // every booking belongs to exactly one page and none go missing.
  const channelBookings = useMemo(
    () => bookings.filter((b) => (b.service_location || 'mobile') === location),
    [bookings, location]
  );
  ...
  // Same four counts the booking_stats view produces, scoped to this channel.
  const stats: BookingStats = useMemo(() => {
    const weekStart = getLocalDateString(startOfWeek());
    const weekEndDate = startOfWeek();
    weekEndDate.setDate(weekEndDate.getDate() + 7);
    const weekEnd = getLocalDateString(weekEndDate);

    return {
      upcoming_count: channelBookings.filter((b) => b.status === 'scheduled').length,
      today_count: channelBookings.filter((b) => b.booking_date === todayStr).length,
      week_count: channelBookings.filter(
        (b) => b.booking_date >= weekStart && b.booking_date < weekEnd
      ).length,
      completed_count: channelBookings.filter((b) => b.status === 'completed').length,
    };
  }, [channelBookings, todayStr]);

  // The list mirrors the Dashboard, which shows the scheduled queue only.
  ...
  const filteredBookings = useMemo(() => {
    return scheduledBookings.filter((b) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-2 border-b border-charcoal-border/40">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-xl sm:text-3xl font-bold tracking-tight text-charcoal">{heading}</h1>
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] sm:text-xs font-semibold bg-sage-100 text-sage-800">
              <BadgeIcon className="w-3 h-3 text-sage-600" />
              {badgeLabel}
            </span>
          </div>
          <p className="text-xs sm:text-sm text-charcoal-muted mt-0.5 sm:mt-1">{description}</p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-sage-50/80 border border-sage-200 text-charcoal text-xs flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-charcoal-muted mt-0.5">
              Add your credentials in <code className="font-mono bg-charcoal-card px-1 py-0.5 rounded border border-sage-200">.env.local</code> to load live bookings.
            </p>
          </div>
        </div>
      )}
      ...
      {/* Metrics Section: 4 StatCards scoped to this channel */}
      <section aria-label="Booking Statistics">
        <StatCardsGrid stats={isLoading ? null : stats} />
      </section>
